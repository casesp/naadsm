 ...
 * given unit, we maintain two linked lists for each herd.  The linked lists
 ...
 * attached to its source herd <i>and</i> from the in-list attached to its
 * recipient herd before it is freed.
 *
...
typedef struct
{
  ...
  double *trace_success[NAADSM_NCONTACT_TYPES]; /**< Probability of tracing a
    ...
    contacts". */
  PDF_dist_t **trace_delay[NAADSM_NCONTACT_TYPES]; /**< Number of days that a
  ...
  unsigned int nherds;          /* Number of units.  Stored here because it is
...
handle_exposure_event (struct naadsm_model_t_ *self, EVT_event_t * e)
{
  ...
  /* Exposures by airborne or local spread can be recorded by exposure-monitor,
   * but they cannot be traced, so nothing should be done with them here. */  
  if (event->traceable && event->exposing_herd != NULL)
    {
      if (NAADSM_UnspecifiedInfectionType == event->contact_type)
      ...
      else
        {
          ...
          g_queue_push_head (local_data->trace_out[event->exposing_herd->index], event_copy);
          g_queue_push_head (local_data->trace_in[event->exposed_herd->index], event_copy);
        }
    }
 ...
 */
void
handle_attempt_to_trace_event (struct naadsm_model_t_ *self,
                               ...
                               RAN_gen_t * rng, EVT_event_queue_t * queue)
{
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- ENTER handle_attempt_to_trace_event (%s)", MODEL_NAME);
#endif
  ...
  g_debug ("attempting %s from unit \"%s\", %i days back",
           NAADSM_trace_direction_name[direction], herd->official_id,
           local_data->trace_period);
#endif

  /* Do the trace.  Note that this process may issue multiple TraceResults for
   * the same pair of herds if A has had contact with B several times in the
   * time period of interest. */
  if (direction == NAADSM_TraceBackOrIn)
    iter = g_queue_peek_head_link (local_data->trace_in[herd->index]);
  else if (direction == NAADSM_TraceForwardOrOut)
    iter = g_queue_peek_head_link (local_data->trace_out[herd->index]);
  else
    iter = NULL;

#if DEBUG
  if (direction == NAADSM_TraceBackOrIn)
    g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "trace list contains %u records",
           g_queue_get_length (local_data->trace_in[herd->index]));
  else if (direction == NAADSM_TraceForwardOrOut)
    g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "trace list contains %u records",
           g_queue_get_length (local_data->trace_out[herd->index]));
#endif
  ...
  for (; iter != NULL; iter = g_list_next (iter))
    {
      ...
      p = local_data->trace_success[record->contact_type][herd->production_type];
      r = RAN_num (rng);
      ...
      if (!trace_successful)
        { 
          ...
          EVT_event_enqueue (queue,
                             EVT_new_trace_result_event (record->exposing_herd,
                                                         record->exposed_herd,
                                                         record->contact_type,
                                                         direction, day, day, FALSE));
          #if DEBUG
            g_debug ("%s misses contact from unit \"%s\" -> unit \"%s\" on day %hu (%i days ago)",
                     NAADSM_trace_direction_name[direction],
                     record->exposing_herd->official_id,
                     record->exposed_herd->official_id, record->day, days_ago);
          #endif
          continue;
        }
      ...
      delay_dist = local_data->trace_delay[record->contact_type][herd->production_type];
      if (delay_dist == NULL)
        delay = 0;
      else
        delay = PDF_random_non_neg_int (delay_dist, rng);

      result = EVT_new_trace_result_event (record->exposing_herd,
                                           record->exposed_herd,
                                           record->contact_type,
...
#if DEBUG
          g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                 "  trace result will be delayed %i days", delay);
#endif
        }
    }                           /* end of iteration over trace list. */

#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- EXIT handle_attempt_to_trace_event (%s)", MODEL_NAME);
#endif
}
     ...
     ZON_zone_list_t * zones)
{
  ...
  local_data->trace_out = g_new0 (GQueue *, local_data->nherds);
  local_data->trace_in = g_new0 (GQueue *, local_data->nherds);
  for (i = 0; i < local_data->nherds; i++)
    {
