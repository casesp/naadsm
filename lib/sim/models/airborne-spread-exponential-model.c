 ...
 *         prevalence in <i>A</i>
 *         \htmlonly &times; \endhtmlonly \latexonly $\times$ \endlatexonly
 *         (probability of infection at 1 km)<sup>distance from <i>A</i> to
 *         <i>B</i></sup>
 *         \htmlonly &times; \endhtmlonly \latexonly $\times$ \endlatexonly
 ...
 * Libtool". */
#define interface_version airborne_spread_exponential_model_LTX_interface_version
#define is_singleton airborne_spread_exponential_model_LTX_is_singleton
#define new airborne_spread_exponential_model_LTX_new
#define set_params airborne_spread_exponential_model_LTX_set_params
#define run airborne_spread_exponential_model_LTX_run
#define reset airborne_spread_exponential_model_LTX_reset
#define events_listened_for airborne_spread_exponential_model_LTX_events_listened_for
#define is_listening_for airborne_spread_exponential_model_LTX_is_listening_for
#define has_pending_actions airborne_spread_exponential_model_LTX_has_pending_actions
#define has_pending_infections airborne_spread_exponential_model_LTX_has_pending_infections
#define to_string airborne_spread_exponential_model_LTX_to_string
#define local_printf airborne_spread_exponential_model_LTX_printf
#define local_fprintf airborne_spread_exponential_model_LTX_fprintf
#define local_free airborne_spread_exponential_model_LTX_free
#define handle_request_for_infection_causes_event airborne_spread_exponential_model_LTX_handle_request_for_infection_causes_event
#define handle_new_day_event airborne_spread_exponential_model_LTX_handle_new_day_event
#define callback airborne_spread_exponential_model_LTX_callback
#define check_and_infect airborne_spread_exponential_model_LTX_check_and_infect
#define events_created airborne_spread_exponential_model_LTX_events_created
 ...
 */
double *
build_size_factor_list_exp (HRD_herd_list_t * herds)
{
  HRD_herd_t *herd;
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- ENTER build_size_factor_list_exp");
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- EXIT build_size_factor_list_exp");
#endif
                  ...
                  EVT_new_day_event_t * event, RAN_gen_t * rng, EVT_event_queue_t * queue)
{
  ...
  if (distance - max_spread > EPSILON)
    {
#if DEBUG
      g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "  herd %s too far (%g > %g)",
             herd2->official_id, distance, max_spread);
#endif
      goto end;
    }
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "  herd %s within range (%g <= %g)",
         herd2->official_id, distance, max_spread);
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "  P = %g * %g * %g * %g",
         herd1_size_factor, herd1->prevalence, distance_factor, herd2_size_factor);
#endif

  P = herd1_size_factor * herd1->prevalence * distance_factor * herd2_size_factor;
  r = RAN_num (rng);
...
#if DEBUG
          g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                 "  r (%g) < P (%g), target herd will be infected on day %i",
                 r, P, event->day + delay);
#endif
        }
    }
...
#if DEBUG
      g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "  r (%g) >= P (%g), target herd not infected", r, P);
#endif
      ;
    }
        ...
        if (to_production_type[j] == TRUE)
          {
             ...
             * (the herd size factors) that may boost the probability by as
             * much as 4 times. */
...
new (scew_element * params, HRD_herd_list_t * herds, ZON_zone_list_t * zones)
{
  ...
  /* Compute the herd size factors. */
  local_data->herd_size_factor = build_size_factor_list_exp (herds);
...
char *
airborne_spread_exponential_model_interface_version (void)
{
...
ergadm_model_t *
airborne_spread_exponential_model_new (scew_element * params, HRD_herd_list_t * herds, ZON_zone_list_t * zones)
{
...
gboolean
airborne_spread_exponential_model_is_singleton (void)
{
  return is_singleton ();
}
