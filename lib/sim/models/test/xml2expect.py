...
expect_after {
	...
	for test in tests:
		...
		print 'set params "test/model.%s/%s.xml"' % (category, parameterFileName)
		print 'set herds "test/%s.xml"' % (herdFileName)
	...
	for test in tests:
		...
		for table in tables:
			...
			for row in table.getElementsByTagName ("tr"):
				...
				for cell in row.getElementsByTagName ("td"):
					state = getText (cell)
					if state_code.has_key(state): # convert long name to short 
						state = state_code[state]
					print state,
				print '}'
			print '  }'
