...
#define handle_commitment_to_destroy_event economic_model_handle_commitment_to_destroy_event
...
#define handle_detection_event economic_model_handle_detection_event
...
#define SOMETIMES MAX(TRUE,FALSE)+1
...
typedef struct
{
  ...
  int appraisal_applies_to_dead_units; /* can have values TRUE, FALSE, SOMETIMES */
  int indemnification_applies_to_dead_units;
  int carcass_disposal_applies_to_dead_units;
  int cleaning_disinfecting_applies_to_dead_units;
}
...
typedef struct
{
  ...
  GHashTable *awaiting_destruction; /**< A table that tracks units that are
    awaiting destruction.  If a unit is awaiting destruction, it will have an
    entry in this table.  Used for determining costs when Dead from Disease
    units are treated like Destroyed ones, but only if they died after being
...
/**
 * Responds to a commitment to destroy event by recording that the unit is
                                    ...
                                    EVT_commitment_to_destroy_event_t * event)
{
  ...
  g_hash_table_insert (local_data->awaiting_destruction, event->herd, GINT_TO_POINTER(1));
...
handle_destruction_event (struct naadsm_model_t_ *self, EVT_destruction_event_t * event)
{
  ...
  g_hash_table_remove (local_data->awaiting_destruction, herd);
...
/**
 * Responds to a detection (as Dead from Disease) event by computing its cost.
 *
 * @param self this module.
...
handle_detection_event (struct naadsm_model_t_ *self, EVT_detection_event_t * event)
{
  ...
  unsigned int size;
  gpointer p;
  gboolean was_awaiting_destruction;
  double cost, sum = 0;
  ...
  p = g_hash_table_lookup (local_data->awaiting_destruction, herd);
  was_awaiting_destruction = (p != NULL);
  if (was_awaiting_destruction)
    {
#if DEBUG
      g_debug ("unit \"%s\" was awaiting destruction", herd->official_id);
#endif
      g_hash_table_remove (local_data->awaiting_destruction, herd);
    }
  ...
  if (herd->status == DeadFromDisease &&
      ...
      local_data->destruction_cost_params[herd->production_type])
    {
      ...
      if (params->appraisal_applies_to_dead_units == TRUE
          || (was_awaiting_destruction && params->appraisal_applies_to_dead_units == SOMETIMES))
        {
          cost = params->appraisal;
          /* RPT_reporting_add_real (local_data->appraisal_cost, cost, NULL); */
          RPT_reporting_add_real (local_data->cumul_appraisal_cost, cost, NULL);
      ...
      if (params->indemnification_applies_to_dead_units == TRUE
          || (was_awaiting_destruction && params->indemnification_applies_to_dead_units == SOMETIMES))
        {
          cost = size * params->indemnification;
          /* RPT_reporting_add_real (local_data->indemnification_cost, cost, NULL); */
          RPT_reporting_add_real (local_data->cumul_indemnification_cost, cost, NULL);
      ...
      if (params->carcass_disposal_applies_to_dead_units == TRUE
          || (was_awaiting_destruction && params->carcass_disposal_applies_to_dead_units == SOMETIMES))
        {
          cost = size * params->carcass_disposal;
          /* RPT_reporting_add_real (local_data->carcass_disposal_cost, cost, NULL); */
          RPT_reporting_add_real (local_data->cumul_carcass_disposal_cost, cost, NULL);
      ...
      if (params->cleaning_disinfecting_applies_to_dead_units == TRUE
          || (was_awaiting_destruction && params->cleaning_disinfecting_applies_to_dead_units == SOMETIMES))
        {
          cost = params->cleaning_disinfecting;
          /* RPT_reporting_add_real (local_data->cleaning_disinfecting_cost, cost, NULL); */
          RPT_reporting_add_real (local_data->cumul_cleaning_disinfecting_cost, cost, NULL);
...
reset (struct naadsm_model_t_ *self)
{
  ...
  */
  ...
  g_hash_table_remove_all (local_data->awaiting_destruction);
...
local_free (struct naadsm_model_t_ *self)
{
  ...
  g_hash_table_destroy (local_data->awaiting_destruction);
...
set_params (struct naadsm_model_t_ *self, PAR_parameter_t * params)
{
  ...
  destruction_cost_data_t destruction_cost_params = { 0 };
  vaccination_cost_data_t vaccination_cost_params = { 0 };
  ...
  e = scew_element_by_name (params, "appraisal-applies-to-dead-units");
  if (e != NULL)
    {
      char *tmp;
      gboolean valid;
      tmp = PAR_get_text (e);
      valid = TRUE;
      if (g_ascii_strncasecmp (tmp, "y", 1) == 0)
        destruction_cost_params.appraisal_applies_to_dead_units = TRUE;
      else if (g_ascii_strncasecmp (tmp, "n", 1) == 0)
        destruction_cost_params.appraisal_applies_to_dead_units = FALSE;
      else if (g_ascii_strncasecmp (tmp, "s", 1) == 0)
        destruction_cost_params.appraisal_applies_to_dead_units = SOMETIMES;
      else
        valid = FALSE;
      has_destruction_cost_params = (has_destruction_cost_params || valid);
      g_free (tmp);
    }
  else
    destruction_cost_params.appraisal_applies_to_dead_units = FALSE;

  e = scew_element_by_name (params, "indemnification-applies-to-dead-units");
  if (e != NULL)
    {
      char *tmp;
      gboolean valid;
      ...
      valid = TRUE;
      if (g_ascii_strncasecmp (tmp, "y", 1) == 0)
        destruction_cost_params.indemnification_applies_to_dead_units = TRUE;
      else if (g_ascii_strncasecmp (tmp, "n", 1) == 0)
        destruction_cost_params.indemnification_applies_to_dead_units = FALSE;
      else if (g_ascii_strncasecmp (tmp, "s", 1) == 0)
        destruction_cost_params.indemnification_applies_to_dead_units = SOMETIMES;
      else
        valid = FALSE;
      has_destruction_cost_params = (has_destruction_cost_params || valid);
      g_free (tmp);
  ...
  e = scew_element_by_name (params, "carcass-disposal-applies-to-dead-units");
  if (e != NULL)
    {
      char *tmp;
      gboolean valid;
      ...
      valid = TRUE;
      if (g_ascii_strncasecmp (tmp, "y", 1) == 0)
        destruction_cost_params.carcass_disposal_applies_to_dead_units = TRUE;
      else if (g_ascii_strncasecmp (tmp, "n", 1) == 0)
        destruction_cost_params.carcass_disposal_applies_to_dead_units = FALSE;
      else if (g_ascii_strncasecmp (tmp, "s", 1) == 0)
        destruction_cost_params.carcass_disposal_applies_to_dead_units = SOMETIMES;
      else
        valid = FALSE;
      has_destruction_cost_params = (has_destruction_cost_params || valid);
      g_free (tmp);
  ...
  e = scew_element_by_name (params, "cleaning-disinfecting-applies-to-dead-units");
  if (e != NULL)
    {
      char *tmp;
      gboolean valid;
      ...
      valid = TRUE;
      if (g_ascii_strncasecmp (tmp, "y", 1) == 0)
        destruction_cost_params.cleaning_disinfecting_applies_to_dead_units = TRUE;
      else if (g_ascii_strncasecmp (tmp, "n", 1) == 0)
        destruction_cost_params.cleaning_disinfecting_applies_to_dead_units = FALSE;
      else if (g_ascii_strncasecmp (tmp, "s", 1) == 0)
        destruction_cost_params.cleaning_disinfecting_applies_to_dead_units = SOMETIMES;
      else
        valid = FALSE;
      has_destruction_cost_params = (has_destruction_cost_params || valid);
      g_free (tmp);
     ...
     ZON_zone_list_t * zones)
{
  ...
  */
  ...
  local_data->awaiting_destruction = g_hash_table_new (g_direct_hash, g_direct_equal);
