 ...
 *
 * @author Neil Harvey <neilharvey@canada.com><br>
...
typedef struct
{
  ...
  GHashTable *detection_status; /**< A table recording detected units.  We only
    ...
    Detection or not. */
  GHashTable *last_test_day; /**< A table recording the last day on which a
    unit was tested.  This is used to prevent multiple tests from occurring
    on the same day. The key is a unit (HRD_herd_t *), the associated data is an
    int showing the last day on which a test occurred. */
}
local_data_t;
...
handle_before_any_simulations_event (EVT_event_queue_t * queue)
{
  ...
  g_ptr_array_add (means, (gpointer) NAADSM_detection_reason_abbrev[NAADSM_DetectionDiagnosticTest]);
  ...
  while (!g_queue_is_empty (q))
    {
       ...
       * to delete the herd from the detection_status table once we've seen
       * all the events for this herd. */
      if (result->type == EVT_TestResult)
        herd = result->u.test_result.herd;
      else
        herd = result->u.detection.herd;
      if (herd != last_herd && last_herd != NULL)
        g_hash_table_remove (local_data->detection_status, last_herd);
      last_herd = herd;       
      ...
      if (result->type == EVT_TestResult)
        {
          ...
          g_debug ("test result for unit \"%s\" now ready",
                   result->u.test_result.herd->official_id);
#endif
                        ...
                        EVT_detection_event_t *event)
{
  ...
  p = g_hash_table_lookup (local_data->detection_status, herd);
  if (p == NULL)
    {
      /* This unit has not been detected and does not have a test ordered. */
      g_hash_table_insert (local_data->detection_status, herd,
 ...
 */
void
handle_test_event (struct naadsm_model_t_ *self,
		   ...
		   EVT_event_queue_t *queue)
{
...
#endif
  /* These initial values aren't used for anything, except to avoid a compiler warning on Windows */
  positive = FALSE;
  correct = FALSE;
  ...
  /* Was this herd already tested today?  If so, don't test it again. */
  ltdp = g_hash_table_lookup (local_data->last_test_day, herd);
  if( NULL != ltdp ) {
    last_test_day = GPOINTER_TO_INT( ltdp );
    if( last_test_day != event->day ) {
    /* This herd has been tested, but not today.  Record today's date, 
       and complete the test. */  
      g_hash_table_insert ( local_data->last_test_day, herd,
                           GINT_TO_POINTER( event->day ) );      
    }
    else {
      /* This herd was tested once already today.  Don't do it again. */
      goto end;     
    } 
  }
  else {
    /* This herd has not been previously tested.  Record today's date, 
       and complete the test. */
    g_hash_table_insert ( local_data->last_test_day, herd,
                         GINT_TO_POINTER( event->day ) ); 
  }
  ...
  if( positive && correct )
    test_result = NAADSM_TestTruePositive;
  else if( positive && !( correct) )
    test_result = NAADSM_TestFalsePositive;
  else if( !(positive) && correct )
    test_result = NAADSM_TestTrueNegative;
  else
    test_result = NAADSM_TestFalseNegative;

  result = EVT_new_test_result_event (herd, event->day, positive, correct, event->reason);
  if (positive)
    detection = EVT_new_detection_event (herd, event->day,
                                         NAADSM_DetectionDiagnosticTest,
                                         NAADSM_detection_reason_abbrev[NAADSM_DetectionDiagnosticTest],
                                         test_result);
  else
    detection = NULL;
  delay = PDF_random_non_neg_int (local_data->delay, rng);
  ...
  if (delay == 0)
  ...
  else
    {
      ...
      if (delay > local_data->pending_results->len)
        naadsm_extend_rotating_array (local_data->pending_results, delay,
                                      local_data->rotating_index);
...
reset (struct naadsm_model_t_ *self)
{
  ...
  g_hash_table_remove_all (local_data->last_test_day);
...
local_free (struct naadsm_model_t_ *self)
{
  ...
  g_hash_table_destroy (local_data->last_test_day);
  g_free (local_data);
     ...
     ZON_zone_list_t *zones)
{
  ...
  local_data->production_type = naadsm_read_prodtype_attribute (params, "production-type", herds->production_type_names);
  ...
  if (e != NULL)
    ...
    local_data->last_test_day = g_hash_table_new (g_direct_hash, g_direct_equal);
