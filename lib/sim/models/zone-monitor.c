...
{
  int nzones;
  projPJ projection; /* The map projection used to convert the herds' latitudes
  ...
  /* Old diagnostic variables moved out of zone-model.c.  These do not
   * correspond to any variables listed in the NAADSM User's Guide. */
  ...
  for (i = 0; i < local_data->nzones - 1; i++)
    {
          ...
          if (perimeter > 0
                  ...
                  || (perimeter > RPT_reporting_get_real1 (local_data->max_perimeter, zone->name))))
            {
              ...
              RPT_reporting_set_integer1 (local_data->max_perimeter_day, event->day, zone->name);
			    ...
			    if (NULL != naadsm_record_zone_perimeter)
			     naadsm_record_zone_perimeter (zone->level, perimeter);           
        }
      ...
      if (local_data->num_holes_filled->frequency != RPT_never)
        RPT_reporting_set_integer1 (local_data->num_holes_filled,
          ...
          if (herd->status != Destroyed)
            {
              ...
              RPT_reporting_add_integer1 (local_data->num_animal_days, herd->size, zone->name);
 ...
 */
void
handle_last_day_event (struct naadsm_model_t_ *self, HRD_herd_list_t * herds,
     ...
     ZON_zone_list_t * zones, EVT_event_t * event, RAN_gen_t * rng, EVT_event_queue_t * queue)
{
  ...
  switch (event->type)
    ...
    case EVT_NewDay:
      ...
      break;
    case EVT_LastDay:
      handle_last_day_event (self, herds, zones, &(event->u.last_day));
      break;
    default:
      g_error
     ...
     ZON_zone_list_t * zones)
{
  ...
  local_data->num_fragments = RPT_new_reporting ("numFragments", RPT_group, RPT_never);
  local_data->num_holes_filled = RPT_new_reporting ("numHolesFilled", RPT_group, RPT_never);
  local_data->cumul_num_holes_filled = RPT_new_reporting ("cumulativeNumHolesFilled", RPT_group, RPT_never);
  ...
  g_ptr_array_add (self->outputs, local_data->num_fragments);
  g_ptr_array_add (self->outputs, local_data->num_holes_filled);
  g_ptr_array_add (self->outputs, local_data->cumul_num_holes_filled);
  ...
  for (i = 0; i < n; i++)
    {
      ...
      /* Old diagnostic variables, always broken down by production type. */
      else if ((strcmp (variable_name, "numFragments") == 0)
               || (strcmp (variable_name, "num-fragments") == 0))
        RPT_reporting_set_frequency (local_data->num_fragments, freq);
      else if ((strcmp (variable_name, "numHolesFilled") == 0)
               || (strcmp (variable_name, "num-holes-filled") == 0))
        RPT_reporting_set_frequency (local_data->num_holes_filled, freq);
      else if ((strcmp (variable_name, "cumulativeNumHolesFilled") == 0)
               || (strcmp (variable_name, "cumulative-num-holes-filled") == 0))
        RPT_reporting_set_frequency (local_data->cumul_num_holes_filled, freq);
      else
  ...
  for (i = 0; i < local_data->nzones; i++)
    {
      ...
      if (i < local_data->nzones - 1)
        {
          ...
          RPT_reporting_set_integer1 (local_data->num_fragments, 0, zone->name);
          RPT_reporting_set_integer1 (local_data->num_holes_filled, 0, zone->name);
          RPT_reporting_set_integer1 (local_data->cumul_num_holes_filled, 0, zone->name);
        }
      ...
      for (j = 0; j < nprod_types; j++)
        {
          drill_down_list[1] = (char *) g_ptr_array_index (herds->production_type_names, j);
