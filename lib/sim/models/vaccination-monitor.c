  ...
  if (strcmp (event->reason, "Ini") != 0)
    {
      ...
      if (RPT_reporting_is_null1 (local_data->first_vaccination_by_prodtype, herd->production_type_name))
        RPT_reporting_set_integer1 (local_data->first_vaccination_by_prodtype, event->day, herd->production_type_name);  
     ...
     ZON_zone_list_t * zones)
{
  ...
  local_data->cumul_num_animals_vaccinated_by_reason_and_prodtype =
    RPT_new_reporting ("vaccA", RPT_group, RPT_never);
  g_ptr_array_add (m->outputs, local_data->vaccination_occurred);
  g_ptr_array_add (m->outputs, local_data->first_vaccination);
  g_ptr_array_add (m->outputs, local_data->first_vaccination_by_reason);
  g_ptr_array_add (m->outputs, local_data->first_vaccination_by_prodtype);
  g_ptr_array_add (m->outputs, local_data->first_vaccination_by_reason_and_prodtype);
