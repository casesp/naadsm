/** @file ring-vaccination-model.c
 * Module that simulates a policy of vaccinating herds within a certain
 * distance of a diseased herd.
 *
...
check_and_choose (int id, gpointer arg)
{
  ...
  /* Is herd 2 already dead or destroyed? */
  if (herd2->status == Destroyed
      || herd2->status == DeadFromDisease)
    goto end;

  /* Has herd 2 been vaccinated recently?  If so, don't queue it again. */
  scorecard = naadsm_get_or_create_scorecard (herd2);
  
  if (callback_data->day < (scorecard->min_next_vaccination_day - 1) )
    {
#if DEBUG
      g_debug ("unit \"%s\" should not be revaccinated until day %d, will not queue on day %d.",
               herd2->official_id, scorecard->min_next_vaccination_day, callback_data->day );
#endif
      /* AR This block can probably be removed the next time anyone reads this function...
      char s[1024];
      sprintf( s, "Herd %d should not be revaccinated on day %d, will be revaccinated no later than %d",
        herd2->index, callback_data->day, scorecard->min_next_vaccination_day );
      naadsm_printf( s );
      */ 
