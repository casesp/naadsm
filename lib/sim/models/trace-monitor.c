...
handle_trace_result_event (struct naadsm_model_t_ *self, EVT_trace_result_event_t *event)
{
  ...
  herd = (event->direction == NAADSM_TraceForwardOrOut) ? event->exposed_herd : event->exposing_herd;
  ...
  drill_down_list[1] = local_data->production_type_name_with_p[herd->production_type];
  ...
  if (local_data->nunits_potentially_traced_by_prodtype->frequency != RPT_never)
    RPT_reporting_add_integer1 (local_data->nunits_potentially_traced_by_prodtype,
                                1, local_data->production_type_name_with_p[herd->production_type]);
  ...
  if (local_data->cumul_nunits_potentially_traced_by_contacttype_and_prodtype->frequency != RPT_never)
    RPT_reporting_add_integer (local_data->cumul_nunits_potentially_traced_by_contacttype_and_prodtype,
                               1, drill_down_list);
  RPT_reporting_add_integer (local_data->nanimals_potentially_traced, herd->size, NULL);
  if (local_data->nanimals_potentially_traced_by_contacttype->frequency != RPT_never)
    RPT_reporting_add_integer1 (local_data->nanimals_potentially_traced_by_contacttype,
                                herd->size, local_data->contact_type_name_with_p[event->contact_type]);
  if (local_data->nanimals_potentially_traced_by_prodtype->frequency != RPT_never)
    RPT_reporting_add_integer1 (local_data->nanimals_potentially_traced_by_prodtype,
                                herd->size, local_data->production_type_name_with_p[herd->production_type]);
  if (local_data->nanimals_potentially_traced_by_contacttype_and_prodtype->frequency != RPT_never)
    RPT_reporting_add_integer (local_data->nanimals_potentially_traced_by_contacttype_and_prodtype,
                               herd->size, drill_down_list);
  RPT_reporting_add_integer (local_data->cumul_nanimals_potentially_traced, herd->size, NULL);
     ...
     ZON_zone_list_t * zones)
{
  ...
  g_ptr_array_add (m->outputs, local_data->nanimals_potentially_traced);
  g_ptr_array_add (m->outputs, local_data->nanimals_potentially_traced_by_contacttype);
  g_ptr_array_add (m->outputs, local_data->nanimals_potentially_traced_by_prodtype);
  g_ptr_array_add (m->outputs, local_data->nanimals_potentially_traced_by_contacttype_and_prodtype);
  g_ptr_array_add (m->outputs, local_data->cumul_nanimals_potentially_traced);
  g_ptr_array_add (m->outputs, local_data->cumul_nanimals_potentially_traced_by_contacttype);
  g_ptr_array_add (m->outputs, local_data->cumul_nanimals_potentially_traced_by_prodtype);
  g_ptr_array_add (m->outputs, local_data->cumul_nanimals_potentially_traced_by_contacttype_and_prodtype);
  g_ptr_array_add (m->outputs, local_data->nanimals_traced);
  g_ptr_array_add (m->outputs, local_data->nanimals_traced_by_contacttype);
  g_ptr_array_add (m->outputs, local_data->nanimals_traced_by_prodtype);
  g_ptr_array_add (m->outputs, local_data->nanimals_traced_by_contacttype_and_prodtype);
  g_ptr_array_add (m->outputs, local_data->cumul_nanimals_traced);
  g_ptr_array_add (m->outputs, local_data->cumul_nanimals_traced_by_contacttype);
  g_ptr_array_add (m->outputs, local_data->cumul_nanimals_traced_by_prodtype);
  g_ptr_array_add (m->outputs, local_data->cumul_nanimals_traced_by_contacttype_and_prodtype);
