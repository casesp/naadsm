...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- ENTER ZON_zone_list_append");
#endif
...
#if DEBUG
          g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                 "there is already a background zone named \"%s\", ignoring zone \"%s\"",
                 existing_zone->name, zone->name);
#endif
          ;
        }
...
#if DEBUG
          g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                 "zones \"%s\" and \"%s\" have the same level (%i)",
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
         "new zone \"%s\" will be inserted into list at position %i", zone->name, i);
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- EXIT ZON_zone_list_append");
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- ENTER ZON_zone_list_reset");
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- EXIT ZON_zone_list_reset");
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- ENTER ZON_zone_list_add_focus");
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- EXIT ZON_zone_list_add_focus");
#endif
}
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- ENTER ZON_fprintf_zone_list");
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- EXIT ZON_fprintf_zone_list");
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- ENTER ZON_free_zone_list");
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- EXIT ZON_free_zone_list");
#endif
  return;
}
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
         "background zone is named \"%s\", has %i fragments",
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- ENTER ZON_new_zone");
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- ENTER ZON_clone_zone");
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- ENTER ZON_zone_to_gnuplot");
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- EXIT ZON_zone_to_gnuplot");
#endif
...
ZON_zone_add_focus (ZON_zone_t * zone, double x, double y, gpc_polygon ** holes)
{
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
         "creating %i-sided polygon to approximate a circle", CIRCLE_NSIDES);
#endif
  ...
  while (i < intermedpoly->num_contours)
    if (intermedpoly->hole[i] == TRUE)
      {
#if DEBUG
        g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "removing hole (contour at index %i)", i);
#endif
        if (holes != NULL)
          {
...
#if DEBUG
      g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "union new polygon with itself");
#endif
...
#if DEBUG
      g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
             "new circle is disjoint from any existing zone fragment");
#endif
...
#if DEBUG
          g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                 "contour %i %ith point=(%.2f,%.2f) %ith point=(%.2f,%.2f)",
                 i,
                 CIRCLE_NSIDES / 4 - 1,
...
#if DEBUG
      g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "new contour has index %i", i);
#endif
...
#if DEBUG
          g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                 "fragment %i now maps to contour with index %i", fragment->id, j);
#endif
          if (j == i)
            fragment_containing_focus = fragment;
        }
  ...
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "to visualize new zone:\n%s", s);
  g_free (s);
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- EXIT ZON_zone_add_focus");
#endif
