...
/**
 * Retrieves a nonNegativeInteger value.
 *
 * Side effect: upon return, the location indicated by <i>success</i> contains
 * FALSE if the parameter was missing, out of range, or otherwise invalid, and
 * TRUE otherwise.
 *
 * @param param a nonNegativeInteger parameter.
 * @param success a location in which to store a success or failure flag.
 * @return the nonNegativeInteger.  If the conversion did not succeed, this
 *   value is undefined.
 */
unsigned long
PAR_get_non_negative_integer (PAR_parameter_t * param, gboolean * success)
{
  scew_element *e;
  unsigned long x;
  const char *text;
  ...
  if( old_style_xml ) 
    {
      ...
      #if DEBUG
        g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "%u bins", nbins);
      #endif
