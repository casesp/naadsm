...
/** @file smherds2xml/parser.c
 * A parser for SpreadModel 2.14 herd status snapshot files.
 *
...
/** @page fileformat SpreadModel 2.14 herd status snapshot file format
 ...
 * comma-separated.
 *
 * \em Lat and \em Lon are decimal numbers.  That is, 100°30'00"W would be
 * given as -100.5.
 */
...
%token HERDSIZE LAT LON STATUS DINSTATUS DLINSTATUS
...
header :
    HERDSIZE COMMA LAT COMMA LON COMMA STATUS COMMA DINSTATUS COMMA
    DLINSTATUS COMMA exposure_fields_list COMMA HOWINFECTED COMMA
    DSINCEDET COMMA DSINCEVAC COMMA HOLDFOR COMMA HOLDDAYS COMMA HOLDPRIORITY
    {
      ntracked_exposures = $13;
      nfields = 6 + $13*3 + 6;
    }
  ;
...
herd:
    INT COMMA FLOAT COMMA FLOAT COMMA INT COMMA INT COMMA INT COMMA int_list
    {
      int length;
      ...
      $$ = HRD_new_herd (0, "Unknown livestock type", $1, $3, $5);
      assert ($$ != NULL);
      $$->status = $7;
