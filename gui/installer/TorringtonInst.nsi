...
# Project: NSIS installer script for NAADSM, experimental version "Torrington"
...
Name "NAADSM-Torrington 3.2.17"

#NOTE: Can use the 'Icon' command to specify an icon file(.ico) for the .exe file.
Icon installerIcon.ico

#Installer Name
outFile "TorringtonSetup.exe"

#Default Installation Directory
installDir "$PROGRAMFILES\NAADSM-Torrington 3.2.17"
...
page custom startDialog "" ": Start Dialog"
 Function startDialog
  ...
  NAADSM 'Torrington' version 3.2.17 \ 
...
writeUninstaller $INSTDIR\TorringtonUninstall.exe

#the following 2 commands add uninstall functionality in Add/Remove Programs
WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\NAADSM-Torrington-3.2.17" \
                 "DisplayName" "NAADSM-Torrington 3.2.17"
WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\NAADSM-Torrington-3.2.17" \
                 "UninstallString" "$INSTDIR\TorringtonUninstall.exe"
sectionEnd
...
createShortCut "$SMPROGRAMS\NAADSM-Torrington 3.2.17.lnk" "$INSTDIR\torrington.exe"
sectionEnd
...
createShortCut "$DESKTOP\NAADSM-Torrington 3.2.17.lnk" "$INSTDIR\torrington.exe"
sectionEnd
...
section "Uninstall"
  Delete "$INSTDIR\TorringtonUninstall.exe"
  ...
  Delete "$INSTDIR\torrington.dll"
  Delete "$INSTDIR\UnzDll.dll"
  Delete "$INSTDIR\remote.dll"
  Delete "$INSTDIR\torrington.exe"
  Delete "$INSTDIR\ZipDll.dll"
  Delete "$INSTDIR\libgcc_s_dw2-1.dll"  
  Delete "$INSTDIR\spreadmodel.ini"

  ifFileExists "$SMPROGRAMS\NAADSM-Torrington 3.2.17.lnk" DeleteSMlink DoNothingSM
  DeleteSMlink:
  Delete "$SMPROGRAMS\NAADSM-Torrington 3.2.17.lnk"
  
  DoNothingSM:

  ifFileExists "$DESKTOP\NAADSM-Torrington 3.2.17.lnk" DeleteDesktopLink DoNothingDESK
  DeleteDesktopLink:
  Delete "$DESKTOP\NAADSM-Torrington 3.2.17.lnk"

  DoNothingDESK:

  RMDir $INSTDIR

  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\NAADSM-Torrington-3.2.17"
