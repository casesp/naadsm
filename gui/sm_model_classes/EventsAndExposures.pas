...
----------------------
Begin: 2005/09/01
Last revision: $Date: 2011-09-13 23:23:22 $ $Author: areeves $
Version number: $Revision: 1.25.6.8 $
Project: NAADSM
...
interface
  ...
  type TEventCode = (
    ...
    EVTInfected,
    EVTDetectedClinical,
    EVTDetectedDiagnosticTest,
    EVTDetectedDead,
    EVTDetectedDeadDestr,
    EVTDetectedDeadVacc,
    EVTZoneFocus,
    EVTZoneChanged,
    EVTHerdExam,
  ...
  function eventCodeChar( const code: TEventCode ): string;
  function eventCodeString( const code: TEventCode ): string;
  function eventFromCode( const c: string ): TEventCode;
  function detectionEventCode( const d: TNAADSMDetectionReason ): TEventCode;
...
//-----------------------------------------------------------------------------
  ...
  function eventCodeChar( const code: TEventCode ): string;
    begin
      case code of
        ...
        EVTDetectedClinical: result := 'EC';
        EVTDetectedDiagnosticTest: result := 'ET';
        EVTDetectedDead: result := 'ED';
        EVTDetectedDeadDestr: result := 'EDD';
        EVTDetectedDeadVacc: result := 'EDV';
  ...
  function eventFromCode( const c: string ): TEventCode;
    begin
      ...
      else if( 'EC' = c ) then result := EVTDetectedClinical
      else if( 'ET' = c ) then result := EVTDetectedDiagnosticTest
      else if( 'ED' = c ) then result := EVTDetectedDead
      else if( 'EDD' = c ) then result := EVTDetectedDeadDestr
      else if( 'EDV' = c ) then result := EVTDetectedDeadVacc
  ...
  function eventCodeString( const code: TEventCode ): string;
    begin
      case code of
        ...
        EVTDetectedClinical: result := tr( 'Detection based on clinical signs' );
        EVTDetectedDiagnosticTest: result := tr( 'Detection based on diagnostic test' );
        EVTDetectedDead: result := tr( 'Detection based on death from disease' );
        EVTDetectedDeadDestr: result := tr( 'Detection by destruction crew based on death from disease' );
        EVTDetectedDeadVacc: result := tr( 'Detection by vaccination crew based on death from disease' );
  ...
  function detectionEventCode( const d: TNAADSMDetectionReason ): TEventCode;
    begin
      case d of
        NAADSMDetectionClinicalSigns: result := EVTDetectedClinical;
        NAADSMDetectionDiagnosticTest: result := EVTDetectedDiagnosticTest;
        NAADSMDetectionDeadFromDisease: result := EVTDetectedDead;
        NAADSMDetectionDeadFromDiseaseByVaccinationTeam: result := EVTDetectedDeadVacc;
        NAADSMDetectionDeadFromDiseaseByDestructionTeam: result := EVTDetectedDeadDestr;
        else
          begin
            raise exception.create( 'Unrecognized detection reason in detectionEventCode()' );
            result := EVTUnspecified;
          end
        ;
      end;
...
//-----------------------------------------------------------------------------
// TSMExposureOrTrace: construction
//-----------------------------------------------------------------------------
  constructor TSMExposureOrTrace.create(
    ...
    begin
      ...
      _method := naadsmContactCode( e.exposureMethod );
...
//-----------------------------------------------------------------------------
  ...
  class function TSMExposureOrTrace.getExposureCodeString( const code: string ): string;
    begin
      ...
      else if( 'L' = code ) then result := tr( 'Local-area exposure' )
    ...
    var
      ...
      for i := 0 to self.count - 1 do
        begin
          ...
          if( ( 0 = e.day ) and ( not( 'N' = e.method ) ) ) then
            raise exception.Create('There is a problem (1) in TSMExposureList.populateDatabase(): ' + intToStr( e.day ) + ' ' + e.method )
          ;

          if( ( not( 0 = e.day ) ) and ( 'N' = e.method ) ) then
            raise exception.Create('There is a problem (2) in TSMExposureList.populateDatabase(): ' + intToStr( e.day ) + ' ' + e.method )
          ;

          if( ( not( 0 = e.day ) ) and ( not( 'N' = e.method ) ) ) then
            begin
              vDict.clear();
              vDict['iteration'] := e.iteration;
              vDict['day'] := e.day;
