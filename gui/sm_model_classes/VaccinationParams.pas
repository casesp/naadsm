...
Version number: $Revision: 1.55.6.4 $
...
//-----------------------------------------------------------------------------
	function TVaccinationParams.populateDatabase( db: TSMDatabase; ptID: integer ): integer;
      ...
      dict['useVaccination'] := db.sqlBool( useVaccination );
      ...
      if( nil <> pdfVaccImmune ) then
      	dict['vaccImmunePeriodPdfID'] := intToStr( pdfVaccImmune.id )
     		...
     		dict['vaccImmunePeriodPdfID'] := DATABASE_NULL_VALUE
      ;
...
// Overridden from TModelWithFunctions
//-----------------------------------------------------------------------------
  function TVaccinationParams.getChartSet(): TChartSet;
    begin
...
(*
  procedure TVaccinationParams.changeChart(
        const whichChart: TSMChart;
        const oldChartName: string;
        newChart: TChartFunction;
        addlInfo: integer = -1
