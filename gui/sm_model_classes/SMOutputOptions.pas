...
Version number: $Revision: 1.18.4.1 $
Project: NAADSM
...
//-----------------------------------------------------------------------------
  ...
  procedure TSMOutputOptions.populateDatabase( db: TSMDatabase; const updateAction: TDBUpdateActionType );
    ...
    begin
      ...
      qDict['saveAllDailyOutputs'] := usBoolToText( saveAllDailyOutputs );
      qDict['saveDailyOutputsForIterations'] := intToStr( dailyOutputsForIterations );
      qDict['writeDailyStatesFile'] := usBoolToText( writeDailyStatesFile );
      qDict['saveDailyEvents'] := usBoolToText( _saveDailyEvents );
      qDict['saveDailyExposures'] := usBoolToText( _saveDailyExposures );

      if( 0 < length( trim( dailyStatesFileName ) ) ) then
        qDict['dailyStatesFileName'] := db.sqlQuote( dailyStatesFileName )
      ;

      qDict['writeNAADSMapOutput'] := usBoolToText( writeNAADSMapOutput );
