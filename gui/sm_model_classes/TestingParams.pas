...
-----------------
Begin: 2008/04/25
Last revision: $Date: 2011-08-22 23:09:51 $ $Author: areeves $
Version number: $Revision: 1.8.4.6 $
Project: NAADSM
    ...
    begin
      ...
      if( nil <> pdfTestDelay ) then
      	dict['testDelayPdfID'] := intToStr( pdfTestDelay.id )
     		...
     		dict['testDelayPdfID'] := 'NULL'
      ;

      dict['testDirectForward'] := usBoolToText( testDirectForward );
      dict['testIndirectForward'] := usBoolToText( testIndirectForward );
      dict['testDirectBack'] := usBoolToText( testDirectBack );
      dict['testIndirectBack'] := usBoolToText( testIndirectBack );
...
//-----------------------------------------------------------------------------
  function TTestingParams.ssXml( const productionTypeID: integer ): string;
    begin
      ...
      result := result + pdfTestDelay.ssXml( 3 );
