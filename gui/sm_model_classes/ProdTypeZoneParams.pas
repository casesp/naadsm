...
----------------------
Begin: 2006/12/19
Last revision: $Date: 2011-03-28 22:01:26 $ $Author: areeves $
Version number: $Revision: 1.4.4.9 $
Project: NAADSM
...
//*******************************************************************************
  ...
  //-----------------------------------------------------------------------------
    ...
    procedure TZoneProdTypeComboParams.setChart( whichChart: TSMChart; fn: TChartFunction );
      ...
      begin
        ...
        case whichChart of
          ZONMovementDirect:
            self.relDirectMovementName := newName
          ;
          ZONMovementIndirect:
            self.relIndirectMovementName := newName
          ;
          else
  ...
  //-----------------------------------------------------------------------------
    ...
    function TZoneProdTypeComboParams.getUpdated(): boolean; begin result := _updated; end;
      ...
      begin
        ...
        if( nil <> _zptList[addlInfo] ) then
          begin
            case whichChart of
              ZONMovementDirect:
                _zptList[addlInfo].relDirectMovementName := newName
              ;
              ZONMovementIndirect:
                _zptList[addlInfo].relIndirectMovementName := newName
              ;
            end;
          end
        else
  ...
  //-----------------------------------------------------------------------------
    function TProdTypeZoneParams.ssXML( const productionTypeID: integer ): string;
      begin
        ...
        //rbh20100803: format below did not pass XML validation by XSD, whitespace between element tags not allowed
        ...
        if( directTraceIsZoneTrigger ) then
          begin
            ...
            ;}
            result := result +
                '  <trace-back-zone-focus-model production-type="' + encodeXml( prodTypeDescr ) + '" production-type-id="' + intToStr( productionTypeID ) + '" contact-type="direct"></trace-back-zone-focus-model>' + endl + endl
            ;
          end
        ;
        ...
        if( indirectTraceIsZoneTrigger ) then
          begin
            ...
            ;}
            result := result +
                '  <trace-back-zone-focus-model production-type="' + encodeXml( prodTypeDescr ) + '" production-type-id="' + intToStr( productionTypeID ) + '" contact-type="indirect"></trace-back-zone-focus-model>' + endl + endl
            ;
          end
        ;
