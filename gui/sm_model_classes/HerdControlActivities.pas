...
-------------------------
Begin: 2009/09/02
Last revision: $Date: 2010-06-26 00:07:43 $ $Author: areeves $
Version number: $Revision: 1.3.6.2 $
Project: NAADSM
...
interface
  ...
  type THerdControlActivities = class
    ...
    public
      ...
      procedure cancelHerdDestruction( const queueDay: integer );
...
implementation
  ...
  procedure THerdControlActivities.cancelHerdDestruction( const queueDay: integer );
    begin
      if( not( _isQueuedForDestr ) ) then
        raise exception.Create( 'Unit is not queued for destruction' )
      else
        _isQueuedForDestr := false
      ;
    end
