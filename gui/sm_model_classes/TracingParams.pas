...
------------------
Begin: 2008/04/23
Last revision: $Date: 2011-03-24 00:56:48 $ $Author: areeves $
Version number: $Revision: 1.8.4.10 $
Project: NAADSM
...
//-----------------------------------------------------------------------------
  function TTracingParams.populateDatabase( db: TSMDatabase; ptID: integer ): integer;
    ...
    begin
      ...
      // Tracing
      dict['traceDirectForward'] := usBoolToText( traceDirectForward );
      dict['traceIndirectForward'] := usBoolToText( traceIndirectForward );
      dict['traceDirectBack'] := usBoolToText( traceDirectBack );
      dict['traceIndirectBack'] := usBoolToText( traceIndirectBack );
      ...
      if( nil <> pdfTraceDelay ) then
      	dict['traceDelayPdfID'] := intToStr( pdfTraceDelay.id )
     		...
     		dict['traceDelayPdfID'] := 'NULL'
      ;

    // Exams
    dict['examDirectForward'] := usBoolToText( examDirectForward );
    dict['examDirectForwardMultiplier'] := usFloatToStr( examDirectForwardMultiplier );
    dict['examIndirectForward'] := usBoolToText( examIndirectForward );
    dict['examIndirectForwardMultiplier'] := usFloatToStr( examIndirectForwardMultiplier );
    dict['examDirectBack'] := usBoolToText( examDirectBack );
    dict['examDirectBackMultiplier'] := usFloatToStr( examDirectBackMultiplier );
    dict['examIndirectBack'] := usBoolToText( examIndirectBack );
      ...
      if( traceDirectForward ) then
        begin
          ...
          result := result
            ...
            + ' contact-type="direct" direction="out">' + endl
            + '  </trace-quarantine-model>' + endl + endl
          ;}
          result := result
            + '  <trace-quarantine-model production-type="' + encodeXml( prodTypeDescr ) + '" production-type-id="' + intToStr( productionTypeID ) + '"'
