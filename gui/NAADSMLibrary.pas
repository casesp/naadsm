...
-----------------
Begin: 2004/08/20
Last revision: $Date: 2013-05-14 17:01:47 $ $Author: areeves $
Version: $Revision: 1.23.6.20 $
Project: NAADSM
Website: http://www.naadsm.org
Author: Aaron Reeves <Aaron.Reeves@ucalgary.ca>
--------------------------------------------------
Copyright (C) 2004 - 2013 Colorado State University
...
interface
  ...
  procedure naadsm_cancel_herd_destruction( c: THRDControl ); cdecl; // type TCFnVoid_1_THRDControl
...
implementation
  ...
  var
    ...
    set_cancel_herd_destruction: procedure( fn: TCFnVoid_1_THRDControl ); cdecl;
    ...
    begin
      ...
      if( dllHandle >= 32 ) then // library was successfully loaded.  Assign function pointers now.
        begin
          ...
          set_cancel_herd_destruction := GetProcAddress( dllHandle, 'set_cancel_herd_destruction' );
          if( nil <> @set_cancel_herd_destruction ) then
            set_cancel_herd_destruction( @naadsm_cancel_herd_destruction )
          else
            begin
              dllLoadErrors := dllLoadErrors + endl + 'MISSING FUNCTION set_cancel_herd_destruction';
              result := false;
            end
          ;
      ...
      try
        ...
        // Otherwise, an error log file will be created.
          try
            ...
            {$IFDEF DEBUG}
              if( 0 < _badThingsList.Count ) then
                begin
                  dbcout( 'MESSAGES FROM CORE LIBRARY:', true );
  ...
  procedure naadsm_debug( msg: pchar );
    begin
      dbcout( 'NAADSM DLL: ' + msg, DBSHOWMSG );
    end
  ;
...
//-----------------------------------------------------------------------------
  ...
  procedure naadsm_display_g_message( msg: pchar );
    ...
    begin
      {$IFDEF DEBUG}
        ...
        // Uncomment the following line to display debugging messages from the DLL immediately
        //dbcout( s, true );
  ...
  procedure naadsm_iteration_start( it: integer );
    begin
      dbcout( 'NAADSM: ++ naadsm_iteration_start ' + intToStr( it + 1 ), DBSHOWMSG );
  ...
  procedure naadsm_day_start( day: integer );
    begin
      dbcout2( 'NAADSM: ++ naadsm_day_start ' + intToStr( day ), DBSHOWMSG );
  ...
  procedure naadsm_change_herd_state( r: THRDUpdate );
    ...
    begin
      ...
      {$ENDIF}
        ...
        {$IFDEF DEBUG}
          dbcout( 'NAADSM: Herd with index ' + intToStr( r.herdIndex ) + ' changed state to ' + naadsmDiseaseStateStr( r.status ) , DBSHOWMSG );
        {$ENDIF}
  ...
  procedure naadsm_expose_herd( e: THRDExpose );
    ...
    begin
      {$IFDEF DEBUG}
        ...
        if
          ( not( _smSim.includeLocalAreaSpreadGlobal ) )
        and
          ( NAADSMLocalAreaSpread = e.exposureMethod )
        then
          raise exception.Create( 'Local area spread is being conducted when it should be disabled.' )
        ;

        if
          ( NAADSMInitiallyInfected <> e.exposureMethod )
        and
          ( _simDay <> e.day )
        then
          begin
      ...
      {$ENDIF}
        ...
        if
          ( _smSim.outputOptions.saveDailyExposuresAndTraces )
        and
          ( NAADSMAirborneSpread <> e.exposureMethod )
        and
          ( NAADSMLocalAreaSpread <> e.exposureMethod )
  ...
  procedure naadsm_trace_herd( t: THRDTrace );
    ...
    begin
      ...
      {$ENDIF}
        {$IFDEF DEBUG}
          //dbcout2( 'NAADSM: Trace attempted for herd with index ' + intToStr( t.identifiedIndex ) + ' for ' + naadsmContactStr( t.contactType ), DBSHOWMSG );
          //dbcout2( '        Origin was herd with index ' + intToStr( t.originIndex ), DBSHOWMSG );
        {$ENDIF}
  ...
  procedure naadsm_examine_herd( e: THRDExam );
   var
    ...
    begin
      ...
      {$ENDIF}
        {$IFDEF DEBUG}
          dbcout( 'NAADSM: Exam for herd with index ' + intToStr( e.herdIndex ) + ' for ' + naadsmContactStr( e.contactType ), DBSHOWMSG );
        {$ENDIF}
  ...
  procedure naadsm_queue_herd_for_vaccination( herdIndex: integer );
    begin
      ...
      {$ENDIF}
        ...
        // Do the important work
        //----------------------
        dbcout( 'Queuing herd ' + intToStr( _herds[ herdIndex ].id ) + ' for vaccination on day ' + intToStr( _simDay ), DBSHOWMSG );
  ...
  procedure naadsm_infect_herd( r: THRDInfect );
    ...
    begin
      ...
      {$ENDIF}
        {$IFDEF DEBUG}
          dbcout( 'NAADSM: Herd with index ' + intToStr( r.herdIndex ) + ' was infected.', DBSHOWMSG );
        {$ENDIF}
  ...
  procedure naadsm_detect_herd( d: THRDDetect );
    ...
    begin
      ...
      {$ENDIF}
        ...
        dbcout(
          'NAADSM: Herd with id ' + intToStr( h.id )
            + ' was detected ' + naadsmDetectionReasonStr( d.reason )
            + ' on day ' + intToStr( _simDay )
          , DBSHOWMSG
        );
        ...
        if( assigned( _naadsMap ) ) then _naadsMap.herdEvent( h, detectionEventCode( d.reason ), _simDay );
        ...
        if( _smSim.outputOptions.saveDailyEvents ) then
          begin
            ...
            evt := TSMEvent.create( _eventCounter, _simIteration, _simDay, h.id, zoneID, detectionEventCode( d.reason ) );
            _eventList.append( evt );
            inc( _eventCounter );
  ...
  procedure naadsm_destroy_herd( c: THRDControl );
    ...
    begin
      {$IFDEF DEBUG}
        ...
        //dbcout2( 'NAADSM: Destroying herd ' + intToStr( c.herdIndex ) + ' on day ' + intToStr( _simDay ) + ' for reason ' + naadsmControlActivityStr( c.reason ), DBSHOWMSG );
      {$ENDIF}
  ...
  procedure naadsm_cancel_herd_destruction( c: THRDControl );
    begin
      {$IFDEF DEBUG}
        dbcout( 'NAADSM: ++ Start naadsm_cancel_herd_destruction', DBSHOWMSG );

        // Do some error checking
        //-----------------------
        if( not( _smSim.includeDestructionGlobal ) and ( -1 <> _simDay ) ) then
          raise exception.Create( 'Destruction is being canceled when it should be disabled' )
        ;
      {$ENDIF}
      ...
      {$ENDIF}
        // Do the important work
        //----------------------
        dbcout( 'NAADSM: Canceling destruction for herd ' + intToStr( _herds[ c.herdIndex ].id ) + ' on day ' + intToStr( _simDay ), DBSHOWMSG );

        _herds[c.herdIndex].cancelDestruction( c, _simDay );
      ...
      {$IFDEF DEBUG}
        ...
        dbcout( 'NAADSM: Exception caught in naadsm_cancel_herd_destruction', true );
      end;
      {$ENDIF}

      {$IFDEF DEBUG}
        dbcout( 'NAADSM: -- End naadsm_cancel_herd_destruction', DBSHOWMSG );
      {$ENDIF}
    end
  ;
