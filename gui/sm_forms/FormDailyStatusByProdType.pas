...
---------------------------------
Begin: 2005/06/28
Last revision: $Date: 2011-08-23 21:40:52 $ $Author: areeves $
Version: $Revision: 1.45.6.5 $
Project: NAADSM
...
interface
  ...
  uses
    ...
    FrameChartBase,
    FrameDailyStatusByProdTypeDiseaseStatus,
    FrameDailyStatusByProdTypeControlStatus,
    FrameDailyStatusByProdTypeDetectionStatus
  ;
  ...
  type  TFormDailyStatusByProdType = class( TFormSMOutputBase )
    ...
    fraDiseaseStatus: TFrameDailyStatusByProdTypeDiseaseStatus;
    fraControlStatus: TFrameDailyStatusByProdTypeControlStatus;
    fraDetectionStatus: TFrameDailyStatusByProdTypeDetectionStatus;
            ...
            begin
              ...
              while( nil <> row ) do
                begin
                  ...
                  if( null <> row.field('tsdUDead') ) then inc( _data.tsdUDead, longint( row.field('tsdUDead') ) );
                  ...
                  if( null <> row.field('detnUDead') ) then inc( _data.detnUDead, longint( row.field('detnUDead') ) );
                  //AR:deadDV
                  if( null <> row.field('detnUDeadD') ) then inc( _data.detnUDeadD, longint( row.field('detnUDeadD') ) );
                  if( null <> row.field('detnUDeadV') ) then inc( _data.detnUDeadV, longint( row.field('detnUDeadV') ) );
