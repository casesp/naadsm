...
//-----------------------------------------------------------------------------
  ...
  procedure TFrameEpiIterationSummary.SetupGrids();
    begin
      ...
      fraSgSurv.Cells[0,2] := tr( 'Direct forward traces (successfully traced)*' );
      fraSgSurv.Cells[0,3] := tr( 'Indirect forward traces (successfully traced)*' );
      fraSgSurv.Cells[0,4] := tr( 'Direct backward traces (successfully traced)*' );
      fraSgSurv.Cells[0,5] := tr( 'Indirect backward traces (successfully traced)*' );

      // Fix Me: 20091014 Which of these labels should end in " *" ? Arron said he will revisit this week, else remind him
      // Herd Exams
      fraSgSurv.Cells[0,6] := tr( 'Herd exams from direct forward traces' );
      fraSgSurv.Cells[0,7] := tr( 'Herd exams from indirect forward traces' );
      fraSgSurv.Cells[0,8] := tr( 'Herd exams from direct backward traces' );
      fraSgSurv.Cells[0,9] := tr( 'Herd exams from indirect backward traces' );

      // Fix Me: 20091014 Which of these labels should end in " *" ? Arron said he will revisit this week, else remind him
      // Diagnostic Testing
      fraSgSurv.Cells[0,10] := tr( 'Diagnostic testing from direct forward traces' );
      fraSgSurv.Cells[0,11] := tr( 'Diagnostic testing from indirect forward traces' );
      fraSgSurv.Cells[0,12] := tr( 'Diagnostic testing from direct backward traces' );
      fraSgSurv.Cells[0,13] := tr( 'Diagnostic testing from indirect backward traces' );
      fraSgSurv.Cells[0,14] := tr( 'True positive diagnostic test result' );
      fraSgSurv.Cells[0,15] := tr( 'True negative diagnostic test result' );
      fraSgSurv.Cells[0,16] := tr( 'False positive diagnostic test result' );
      fraSgSurv.Cells[0,17] := tr( 'False negative diagnostic test result' );
...
//-----------------------------------------------------------------------------
    ...
    var
      ...
      q := 'SELECT'
          ...
          + ' (infnUAir + infnUDir + infnUInd ) as infnUAll' // Number of new units infected on this day by any source
        + ' FROM'
          + ' outDailyByProductionType'
        + ' WHERE'
          + ' iteration = ' + intToStr( lastIteration )
      ...
      while( nil <> ptit.current() ) do
          ...
          or
            ...
            begin
              ...
              newDetUOnDay := newDetUOnDay
                + ptit.current().currentOutputs.detnUClin
                + ptit.current().currentOutputs.detnUTest
              ;
  ...
  ;
	...
	procedure TFrameEpiIterationSummary.fillGrids();
      ...
      *)
      ...
      // Detection
      //-----------
      fraSgSurv.rowCount := 18; // remember to update
      ...
      // Tracing
      //--------
			fraSgSurv.Cells[1, 2] := intToStr( _data.trcUDirpFwd ) + ' (' + intToStr( _data.trcUDirFwd ) + ')';
			fraSgSurv.Cells[2, 2] := intToStr( _data.trcADirpFwd ) + ' (' + intToStr( _data.trcADirFwd ) + ')';
			fraSgSurv.Cells[1, 3] := intToStr( _data.trcUIndpFwd ) + ' (' + intToStr( _data.trcUIndFwd ) + ')';
			fraSgSurv.Cells[2, 3] := intToStr( _data.trcAIndpFwd ) + ' (' + intToStr( _data.trcAIndFwd ) + ')';
      fraSgSurv.Cells[1, 4] := intToStr( _data.trcUDirpBack ) + ' (' + intToStr( _data.trcUDirBack ) + ')';
			fraSgSurv.Cells[2, 4] := intToStr( _data.trcADirpBack ) + ' (' + intToStr( _data.trcADirBack ) + ')';
			fraSgSurv.Cells[1, 5] := intToStr( _data.trcUIndpBack ) + ' (' + intToStr( _data.trcUIndBack ) + ')';
			fraSgSurv.Cells[2, 5] := intToStr( _data.trcAIndpBack ) + ' (' + intToStr( _data.trcAIndBack ) + ')';

      // Herd Exam
      //-------------
      fraSgSurv.Cells[1, 6] := intToStr( _data.exmcUDirFwd );
			fraSgSurv.Cells[2, 6] := intToStr( _data.exmcADirFwd );
      fraSgSurv.Cells[1, 7] := intToStr( _data.exmcUIndFwd );
			fraSgSurv.Cells[2, 7] := intToStr( _data.exmcAIndFwd );
      fraSgSurv.Cells[1, 8] := intToStr( _data.exmcUDirBack );
			fraSgSurv.Cells[2, 8] := intToStr( _data.exmcADirBack );
      fraSgSurv.Cells[1, 9] := intToStr( _data.exmcUIndBack );
			fraSgSurv.Cells[2, 9] := intToStr( _data.exmcAIndBack );

      // Diagnostic Testing
      //-------------
      fraSgSurv.Cells[1, 10] := intToStr( _data.tstcUDirFwd );
			fraSgSurv.Cells[2, 10] := intToStr( _data.tstcADirFwd );
      fraSgSurv.Cells[1, 11] := intToStr( _data.tstcUIndFwd );
			fraSgSurv.Cells[2, 11] := intToStr( _data.tstcAIndFwd );
      fraSgSurv.Cells[1, 12] := intToStr( _data.tstcUDirBack );
			fraSgSurv.Cells[2, 12] := intToStr( _data.tstcADirBack );
      fraSgSurv.Cells[1, 13] := intToStr( _data.tstcUIndBack );
			fraSgSurv.Cells[2, 13] := intToStr( _data.tstcAIndBack );
      fraSgSurv.Cells[1, 14] := intToStr( _data.tstcUTruePos );
			fraSgSurv.Cells[2, 14] := intToStr( _data.tstcATruePos );
      fraSgSurv.Cells[1, 15] := intToStr( _data.tstcUTrueNeg );
			fraSgSurv.Cells[2, 15] := intToStr( _data.tstcATrueNeg );
      fraSgSurv.Cells[1, 16] := intToStr( _data.tstcUFalsePos );
			fraSgSurv.Cells[2, 16] := intToStr( _data.tstcAFalsePos );
      fraSgSurv.Cells[1, 17] := intToStr( _data.tstcUFalseNeg );
			fraSgSurv.Cells[2, 17] := intToStr( _data.tstcAFalseNeg );

      // Destruction
      //-------------
      fraSgDestr.rowCount := 8; // remember to update
      ...
      // Infection
      //-----------
      fraSgInf.rowCount := 5; // remember to update
