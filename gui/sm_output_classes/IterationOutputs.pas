...
--------------------
Begin: 2005/07/28
Last revision: $Date: 2011-10-14 00:23:11 $ $Author: areeves $
Version number: $Revision: 1.9.2.11 $
Project: NAADSM
    ...
    begin
      ...
      if( 0 < len ) then
        begin
          ...
          self.resultSet( EpiAdqcUAll ).values.resize( len );
          self.resultSet( EpiAdqcUAll ).values.fill( 0.0 );

          self.resultSet( EpiAdqcAAll ).values.resize( len );
          self.resultSet( EpiAdqcAAll ).values.fill( 0.0 );
          ...
          // Running totals for dead from disease - currently no calculated values
          ...
          for i := 0 to len - 1 do
            begin
              // Running totals for adequate exposures
              self.resultSet( EpiAdqcUAll ).values[i] :=
                self.resultSet( EpiAdqcUAir ).values[i]
                + self.resultSet( EpiAdqcUDir ).values[i]
                + self.resultSet( EpiAdqcUInd ).values[i]
                + self.resultSet( EpiAdqcULcl ).values[i]
              ;

              self.resultSet( EpiAdqcAAll ).values[i] :=
                self.resultSet( EpiAdqcAAir ).values[i]
                + self.resultSet( EpiAdqcADir ).values[i]
                + self.resultSet( EpiAdqcAInd ).values[i]
                + self.resultSet( EpiAdqcALcl ).values[i]
              ;
              ...
              self.resultSet( EpiDescUAll ).values[i] :=
                ...
                //AR:deadDV
                + self.resultSet( EpiDescUDcd ).values[i]
              ;
              ...
              self.resultSet( EpiDescAAll ).values[i] :=
                ...
                //AR:deadDV
                + self.resultSet( EpiDescADcd ).values[i]
              ;

              // Running totals for dead from disease - Presently no calculated values
            end
          ;
        end
      ;
    ...
    begin
      ...
      while( nil <> row ) do
        begin
          ...
          outputSet.resultSet( EpiDeswAMax ).values[i] := row.field( 'deswAMax' );
